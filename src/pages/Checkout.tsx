      ...
      <div className="container max-w-md mx-auto px-4 pt-4">
        ...
        {bankSettings.length > 0 && (
          <div className="glass-card rounded-2xl p-4 shadow-soft mb-4">
            ...
            <p className="text-xs text-muted-foreground mb-3">Transfer {formatMVR(totalPrice)} to:</p>
            {bankSettings.map((bank) => (
              <div key={bank.id} className="bg-muted/50 rounded-xl p-3 mb-2">
        ...
        {/* Shipping Form */}
        <form onSubmit={handlePlaceOrder} className="glass-card rounded-2xl p-4 shadow-soft space-y-3">
          <h2 className="font-semibold text-foreground mb-2">Shipping Details</h2>
          <div>
          ...
          <div>
            ...
            <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} placeholder="Special instructions..." className="w-full px-4 py-3 rounded-xl border border-border bg-white focus:outline-none focus:ring-2 focus:ring-accent resize-none h-16" />
          </div>
        </form>
      </div>
