...
const ProductCard = memo(({ product, size = "large", priority = false }: ProductCardProps) => {
  ...
  return (
    <div className="group block cursor-pointer" onClick={handleClick} style={{ perspective: "1000px" }}>
      <div
            ...
            <div className="mb-2">
              ...
              <div className="flex items-center gap-2 mt-1.5">
                {product.old_price && product.old_price > product.price ? (
                  <>
                    <span className="text-primary font-bold text-base">{formatMVR(product.price)}</span>
                    <span className="text-muted-foreground text-xs line-through">{formatMVR(product.old_price)}</span>
                  </>
                ) : (
                  <span className="text-primary font-bold text-base">{formatMVR(product.price)}</span>
                )}
            ...
            <div className="flex-1 overflow-hidden relative">
              <div className="animate-scroll-up hover:pause-animation">
                <p className="text-muted-foreground text-xs leading-relaxed">
                  {product.description ||
                    "Discover the thrill of RC with this amazing product. Built with premium materials for durability and performance. Perfect for enthusiasts of all skill levels."}
                </p>
              </div>
            </div>
